...
| 54 | graphCheckpoints, agentDecisions | Added tables: `graphCheckpoints: threadId, projectId, updatedAt` and `agentDecisions: ++id, projectId, runId, ts, [projectId+ts], [runId+ts]` | The LangGraph writing orchestrator persists its checkpointer per run thread (resume at the last completed superstep) and logs every Editor-agent decision with the state it saw, the action it chose and whether it was legal | None needed |
| 53 | dailyGoals (data only) | In projects with a `generated` chapter whose prose lives in its scenes, rows whose `wordCount` exceeds the manuscript's present size are set to that size | The v52 double count was recorded into the daily history while it was live (functional audit #30); hand-written projects are not touched, so a deliberate cut keeps its number | None |
| 52 | sections (data only) | Clears `content`/`wordCount` on any section whose body is exactly its scenes joined with `<hr>` | Generated chapters carried a copy of their scenes, so every counter reported them at twice their length (functional audit 2026-09-15); the copy is no longer written | None (a hand-edited body never equals the join, so it is never touched) |
| 51 | contentVectors | Added table: `++id, projectId, kind, refId, model, dim, embeddingStatus, updatedAt, &[projectId+kind+refId]` | Embeddings of the story's own entities and scenes for Related / Lookup; `model`+`dim` on every row so a model switch fails loud | None (rebuildable; `indexProject`) |

...
export const useCharacterChatStore = defineStore('characterChat', () => {
  ...
  async function loadSessions(projectId: any) {
  ...
  function setActiveSession(sessionId: any) {
    activeSessionId.value = sessionId
  }

  function addCharacterToSession(characterId: any) {
    if (!activeSession.value) return
  ...
  function removeCharacterFromSession(characterId: any) {
    if (!activeSession.value) return
    activeSession.value.characterIds = activeSession.value.characterIds.filter(
      (id: any) => id !== characterId
    )
    activeSession.value.updatedAt = Date.now()
    scheduleSave()
  }
  ...
  function addMessage(role: any, content: any, characterId: any = null) {
  ...
  function appendToLastMessage(chunk: any) {
    if (!activeSession.value) return
  ...
  function setLastMessageContent(content: any) {
    if (!activeSession.value) return
    const messages = activeSession.value.messages
    if (messages.length === 0) return
  ...
  function markSavedToNotes(messageId: any) {
    if (!activeSession.value) return
    const msg = activeSession.value.messages.find((m: any) => m.id === messageId)
    if (msg) msg.savedToNotes = true
  }

  function markSavedToManuscript(messageId: any) {
    if (!activeSession.value) return
    const msg = activeSession.value.messages.find((m: any) => m.id === messageId)
  ...
  async function removeSession(sessionId: any) {
  ...
  async function removeSessionsContainingCharacter(characterId: any) {
    try {
  ...
  function setStreaming(val: any) {
    isStreaming.value = val
  }

  function setStreamError(err: any) {

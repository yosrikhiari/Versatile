 ...
 */
export function runRegressionCheck(currentResults: any, baseline: any, options: any = {}) {
  ...
  for (const scene of baseline.scenes) {
    if (!currentResults.find((c: any) => c.sceneId === scene.sceneId)) {
 ...
 */
export function assertNoRegressions(report: any, options: any = {}) {
  ...
  for (const sr of report.sceneResults) {
    ...
    for (const [dimName, info] of Object.entries(sr.degradation as any) as Array<[string, any]>) {

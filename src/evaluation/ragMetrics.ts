export function hitRate(results: any, relevantIds: any, k = results.length) {
  if (!results.length || !relevantIds.length) return 0
  const topK = results.slice(0, k)
  return topK.some((r: any) => relevantIds.includes(r.id)) ? 1 : 0
}

export function meanReciprocalRank(results: any, relevantIds: any) {
  if (!results.length || !relevantIds.length) return 0
...
export function averagePrecision(results: any, relevantIds: any) {
  if (!results.length || !relevantIds.length) return 0
  let hits = 0
  let sumPrecision = 0
  for (let i = 0; i < results.length; i++) {
...
export function ndcgAtK(results: any, relevantIds: any, k = results.length) {
  const topK = results.slice(0, k)
...
export function recallAtK(results: any, relevantIds: any, k = results.length) {
  if (!relevantIds.length) return 0
  const topK = results.slice(0, k)
  const retrieved = topK.filter((r: any) => relevantIds.includes(r.id)).length
  return retrieved / relevantIds.length
}

export function precisionAtK(results: any, relevantIds: any, k = results.length) {
  if (!k) return 0
  const topK = results.slice(0, k)
  const relevant = topK.filter((r: any) => relevantIds.includes(r.id)).length
  return relevant / k
}

export function computeAll(results: any, relevantIds: any, opts: any = {}) {

...
export function aggregateMetrics(results: any) {
  ...
  for (const key of keys) {
    const values = results.map((r: any) => r.metrics[key]).filter((v: any) => v !== undefined)
  ...
  agg.passedQueries = results.filter((r: any) => r.metrics.hitRate === 1).length
...
export async function runEvaluation({ searchFn, testCases, projectId, k }: { searchFn: any; testCases: any; projectId: any; k: any }) {
  ...
  for (const tc of testCases) {
    ...
    results.push({
      ...
      retrievedIds: retrieved.map((r: any) => r.id),
      metrics
    })
  }
  return {

...
export function getDimensionNames(workspaceType: any) {
  const dims = EVAL_DIMENSIONS[workspaceType]
  if (!dims) return null
  return Object.keys(dims)
}

export function splitByPeriod(evals: any, recentFraction: any) {
...
export function computeDrift(evals: any, dimensionNames: any, options: any = {}) {
  const {
  ...
  const hasDrift = (Object.values(dimensionDrifts) as any[]).some(
    (d: any) => d.status === 'regression' || d.status === 'improvement'
  )
...
export function computeOverallTrend(evals: any) {
  const sorted = [...evals].sort((a, b) => {
    const ta = a.timestamp || ''
    const tb = b.timestamp || ''
    return ta < tb ? -1 : ta > tb ? 1 : 0
...
export function analyzeWorkspace(evals: any, workspaceType: any, options: any) {
  ...
  const matched = evals.filter((e: any) => {
...
export function generateReport(results: any, options: any) {
  const totalEvals = results.reduce((s: any, r: any) => s + r.evalCount, 0)
  const workspacesWithDrift = results.filter((r: any) => r.drift?.driftDetected && !r.error)
  const regressions = []
  const improvements = []
  const volatilities = []
  ...
  for (const r of results) {
    if (r.error) continue
    for (const [dim, d] of Object.entries(r.drift?.dimensionDrifts || {}) as any) {

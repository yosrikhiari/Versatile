...
export function installGuardrails(options: GuardrailSetupOptions = {}): GroundingService {
  ...
  if (wanted('fact_canon'))
    register('fact_canon', createFactCanonGuard(grounding, { getFactLedger }))
  if (wanted('content_safety'))
    register('content_safety', createContentSafetyGuard({ blockedTerms }))
  if (wanted('pii_leakage')) register('pii_leakage', createPiiGuard())
  if (wanted('cross_turn_consistency'))
    register('cross_turn_consistency', createCrossTurnGuard(grounding, { getPronouns }))

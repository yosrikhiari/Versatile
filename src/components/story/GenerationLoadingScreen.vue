...
<template>
  <div class="flex flex-col items-center justify-center min-h-[400px] w-full max-w-2xl mx-auto font-ui text-text-primary">
      ...
      <div v-for="stage in stages" :key="stage.id" class="flex flex-col">
        <div class="flex items-center gap-4">
          ...
          <div class="w-6 h-6 flex items-center justify-center shrink-0">
            <BaseIcon v-if="getState(stage.id) === 'complete'" name="check" :size="20" class="text-accent fade-in" />
            <BaseIcon v-else-if="getState(stage.id) === 'active'" name="loader-2" :size="20" class="text-accent animate-spin-slow pulse-amber" />
          ...
          <span 
            :class="[
              ...
              getState(stage.id) === 'active' ? 'text-text-primary drop-shadow-[0_0_8px_rgba(234,179,8,0.3)]' : 
              getState(stage.id) === 'complete' ? 'text-text-secondary opacity-80' : 
        ...
        <div v-if="stage.id === 2 && currentStep >= 2" class="pl-10 mt-3 flex flex-wrap gap-2 min-h-[32px]">
          ...
          <div v-if="getState(2) === 'active'" class="text-xs text-text-hint italic py-1 opacity-50 fade-in-slow">
            Waiting for the ether...
          </div>
        </div>

        <!-- Step 3 Content Cascade (Forging the Story Graph) -->
        <div v-if="stage.id === 3 && currentStep >= 3" class="pl-10 mt-3 flex flex-wrap gap-2 min-h-[32px]">
          <TransitionGroup name="fade-stagger">
            <div 

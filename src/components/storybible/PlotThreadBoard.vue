...
async function completeAllWithAI() {
  ...
  try {
    ...
    if (result) {
      ...
      if (
        extractedEntities.value.characters.length > 0 ||
        extractedEntities.value.locations.length > 0
      ) {
...
function parseEntitiesFromNotes(notes) {
  ...
  if (charsMatch && charsMatch[1]) {
    const names = charsMatch[1]
      .split(/[,;]/)
      .map((n) => n.trim())
      .filter((n) => n && n.length > 1 && n.toLowerCase() !== 'none')
    characters.push(...names)
  }

  const locsMatch = notes?.match(/\[Locations:\s*([^\]]+)\]/)
  if (locsMatch && locsMatch[1]) {
    const names = locsMatch[1]
      .split(/[,;]/)
      .map((n) => n.trim())
      .filter((n) => n && n.length > 1 && n.toLowerCase() !== 'none')
...
function scanForEntities() {
  ...
  if (characters.length === 0 && locations.length === 0) {
    alert(
      'No valid characters or locations found in the structured blocks.\n\nMake sure your sections look like:\n[Characters: Marcus, Elena]\n[Locations: The Tavern]'
    )
    return
  }
        ...
        <template #item="{ element: thread }">
          <div class="bg-bg-secondary border border-border-subtle rounded-lg overflow-hidden">
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex-1 min-w-0">
                  ...
                  <div
                    v-if="thread.notes && expandedThreadId !== thread.id"
                    class="text-2xs text-text-hint truncate mt-0.5 ml-5"
                  >
                    {{ thread.notes }}
                  </div>
                </div>
                <BaseIcon
                  :name="expandedThreadId === thread.id ? 'chevron-down' : 'chevron-right'"
                  :size="14"
                  class="text-text-hint ml-2"
                />
              </div>
            </div>
            ...
            <div
              ...
              class="p-3 border-t border-border-subtle bg-bg-tertiary space-y-3"
            >
              <div class="flex items-center gap-2">
                ...
                >
                  <BaseIcon
                    v-if="isGenerating === editingThread?.id && !generatingField"
                    name="loader-2"
                    :size="10"
                    class="animate-spin"
                  />
              ...
              <div
                v-if="enhanceError"
                class="p-2 bg-danger/10 border border-danger/20 rounded text-xs text-danger"
              >
                {{ enhanceError }}
              </div>
              ...
              <div class="flex gap-2">
                <button
                  class="btn-primary flex-1 py-1 text-xs rounded font-ui"

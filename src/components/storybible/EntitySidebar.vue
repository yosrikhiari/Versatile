...
const emit = defineEmits(['drag-start', 'quick-add', 'toggle-sidebar', 'edit-connection', 'delete-connection'])
...
function getEntityConnections(entityType, entityId) {
  const connections = props.edges.filter(edge => {
    const sourceId = String(edge.sourceId)
    const targetId = String(edge.targetId)
    const entityIdStr = String(entityId)
    return (edge.sourceType === entityType && sourceId === entityIdStr) ||
...
const inNetworkCount = computed(() => 
  props.existingNodeIds.length
)
</script>
...
<template>
  <div class="entity-sidebar h-full flex flex-col bg-bg-secondary border-r border-border-subtle">
    <div class="p-3 border-b border-border-subtle">
      ...
      <div class="relative">
        <BaseIcon name="search" :size="14" class="absolute left-2 top-1/2 -translate-y-1/2 text-text-hint" />
        <input
          v-model="searchQuery"
          type="text"
          placeholder="Search..."
          ...
          >
            <div class="flex items-center justify-between">
              ...
              >
                ...
                ></span>
                <span class="text-xs text-text-primary truncate font-ui">{{ entity.name || entity.title }}</span>
              </button>

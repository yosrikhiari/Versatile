  ...
  >
      ...
      <div v-for="group in navGroups" :key="group.label" class="px-2">
        ...
        <button
          ...
          :class="[
            ...
            isActive(item.panel) ? 'text-text-primary' : 'text-text-secondary hover:text-text-primary hover:bg-surface-hover',
          ]"
          :style="{
        ...
        >
          ...
          <BaseIcon
            :name="item.icon"
            :size="18"
            :class="isActive(item.panel) ? 'text-accent' : 'opacity-70 group-hover/item:opacity-100 transition-opacity duration-150'"
          />
    ...
    <div class="shrink-0 px-2 py-2 border-t border-border-subtle">
      <button
        ...
        :style="isActive(item.panel) ? { background: 'rgba(var(--vers-accent-primary-rgb),0.12)' } : {}"
        :title="effectiveCollapsed ? item.label : ''"
        :aria-current="isActive(item.panel) ? 'page' : undefined"
        @click="onNavClick(item.panel)"
      >

...
function findDialogueTag(text: string, quoteStart: number, quoteEnd: number): TagResult | null {
...
function extractQuotedDialogue(text: string): DialogueResult[] {
...
function extractEmDashDialogue(text: string): DialogueResult[] {
  const results: DialogueResult[] = []
...
function findDialogueTagFallback(text: string): TagResult | null {
...
export function detectDialogue(text: string, paragraphIndex: number): DialogueLine[] {
  if (!text || typeof text !== 'string') return []

  const quotedResults = extractQuotedDialogue(text)
  const emDashResults = extractEmDashDialogue(text)

  const dialogueLines: DialogueLine[] = [...quotedResults, ...emDashResults].map((d) => ({
    paragraphIndex,
...
export function detectDialogueBatch(paragraphs: ParagraphInput[]): DialogueLine[] {

function getFullText(story: any) {
  if (story.fullText) return story.fullText
  if (story.scenes && Array.isArray(story.scenes)) {
    return story.scenes.map((s: any) => s.prose || '').join('\n\n')
  }
  return ''
}

function getTitle(story: any) {
...
function exportAsText(story: any) {
...
function exportAsMarkdown(story: any) {
...
async function copyToClipboard(story: any) {

export class GenerationLifecycleService {
  ...
  onRunParallelGeneration: any = null
  onWriteNextBatch: any = null

  async startGeneration(opts: any = {}) {
    ...
    try {
      ...
      const totalScenes = this.scenePlan.value.reduce((sum: any, s: any) => sum + s.scenes.length, 0)
  ...
  async resumeGeneration(projectId: any, opts: any = {}) {
  ...
  async confirmPlan(projectId: any, opts: any = {}) {
    this.phase.value = 'confirming'
  ...
  async completeGeneration(projectId: any) {
    ...
    try {
      const synced = this.writtenScenes.value.filter((s: any) => s && s.content && s.content.trim())
      if (synced.length === 0) {
        this.actLog.appendThought(
          this.currentTaskId.value,
  ...
  async repairFailedScenes(projectId: any, opts: any = {}) {
    ...
    for (const idx of failedIndexes) {
      const planEntry = scenePlan.reduce((acc: any, section: any) => {
      ...
      const feedback = failed.find((f: any) => f.index === idx)
      this.actLog.appendThought(
        this.currentTaskId.value,
...
IMPORTANT: Return JSON with keys: title, content (1500-2500 words), overview, wordCount, entities, summary`,
      ...
      if (result && result.content && result.content.trim().length >= 200) {
        const existing = this.structuredResults.find((r: any) => r && r.index === idx)
        if (existing) {
    ...
    this.writtenScenes.value = this.structuredResults
      .filter((r: any) => r && r.content && r.content.trim())

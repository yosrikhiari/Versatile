...
function buildEntityBlock(entities: any, label: any, formatter: any) {
  if (entities.length === 0) return ''
  const heading = LABEL_HEADINGS[label] || 'ACTIVE PLOT THREADS:'
  return `\n\n${heading}\n${entities.map(formatter).join('\n')}`
}

function toTraitsArray(val: any) {
...
function formatPlotThread(t: any) {

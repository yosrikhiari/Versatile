...
export function useDriftTriggeredEval(sceneEval: any) {
  const driftMonitor = useDriftMonitor()
  const triggeredActions = ref<any[]>([])
  const isChecking = ref(false)
  const lastCheckResult = ref<any>(null)
  ...
  async function check({
    ...
    try {
      ...
      const regressedDims = Array.from(new Set(regressions.map((r: any) => r.dimension)))

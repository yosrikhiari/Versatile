import { ref } from 'vue'
import { db as _db } from '../services/db-core'
const db = _db as any
import {
...
function splitText(text: any, maxSegmentSize = 300000) {
...
export function useResearchDocuments(projectId: any) {
  ...
  const pendingImportInfo = ref<{ files: any[]; totalChars: number }>({ files: [], totalChars: 0 })
  const truncationInfo = ref<string | null>(null)
  ...
  function checkFileSizes(files: any) {
    let totalChars = 0
    const oversized = []
  ...
  async function removeDocument(id: any) {
    clearDocumentProgress(id)
    await deleteResearchDocument(id)
    await loadDocuments()
  }

  async function importFromUrl(url: any) {
    importError.value = null
  ...
  async function getDocumentChunks(documentId: any) {
  ...
  async function reindexDocument(documentId: any) {
  ...
  async function extractEntities(documentId: any) {

...
export function useNovelPipeline() {
  ...
  const stageState = ref<any>(null)

  async function refreshStages(projectId: any) {
   ...
   */
  async function generate(seed: any, callbacks: any = {}) {
   ...
   */
  async function resume(seed: any, callbacks: any = {}) {
  ...
  async function getResumable(projectId: any) {

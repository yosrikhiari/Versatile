...
export const useVoiceFromManuscript = () => {
  ...
  const extractVoiceProfile = async () => {
    try {
      ...
      if (!manuscript || manuscript.length === 0) {
        addToast('No manuscript text found. Write some prose to extract your voice profile.', 'info');
        return null;
      }
      ...
      if (!profile) {
        addToast('Manuscript too short for voice extraction (minimum 500 words). Keep writing to build your profile.', 'warning');
        return null;
      }
      ...
      addToast(`Voice profile extracted from ${profile.metadata.totalWords} words (confidence: ${(profile.metadata.confidence * 100).toFixed(0)}%)`, 'success');
  ...
  const addSupplementarySample = async (sampleText) => {
    try {
      ...
      mergedProfile.manuscriptSizeAtExtraction = storyBibleStore.voiceProfile?.manuscriptSizeAtExtraction ||
        manuscript.length;
      ...
      addToast(`Profile updated (merge #${mergedProfile.supplementaryMergeCount}). New confidence: ${(mergedProfile.metadata.confidence * 100).toFixed(0)}%`, 'success');
  ...
  const toggleProfileLock = () => {
    ...
    addToast(newState ? 'Voice profile locked. Refresh disabled.' : 'Voice profile unlocked. Auto-refresh enabled.', 'info');
    return newState;
  };

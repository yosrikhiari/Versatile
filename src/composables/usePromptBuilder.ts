 ...
 * @returns {string}
 */
function buildBriefSection(sceneBrief: any) {
  const lines =
 ...
 * @returns {string}
 */
function buildJsonOutputInstructions(_sceneId: any) {
...
function buildSystemPrompt({
  ...
  antiPatterns: string
  activeCraftRules: string
  pastEvalResults: string | null
  proseStyleGuide?: string
  focusInstructions?: string | null
  profileStyleGuide?: string
  voiceConstraint?: string
  promptOverrides?: Record<string, string>
  personaBlock?: string
}) {
  const P = DOCUMENT_PROMPTS as Record<string, any>
  const base = P[categoryType] || P.creative
  const activePrompts: Record<string, string> = { ...base }

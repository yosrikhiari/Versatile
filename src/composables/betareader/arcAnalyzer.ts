...
export async function analyzeArc(scenes: any, aiOptions: any) {
  ...
  const sceneById: Record<number, any> = {}
  for (const s of scenes) {
    sceneById[s.sceneNumber] = s
  }

  const setupPayoffs = ((parsed as any).setupPayoffs || []).map((sp: any, i: any) => ({
    id: `arc-sp-${i}`,
  ...
  const droppedThreads = ((parsed as any).droppedThreads || []).map((dt: any, i: any) => ({
    id: `arc-dt-${i}`,
  ...
  const pacing = ((parsed as any).pacing || []).map((p: any, i: any) => ({
    id: `arc-pace-${i}`,

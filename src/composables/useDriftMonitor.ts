...
import { analyzeWorkspace, generateReport, DEFAULTS } from '../evaluation/driftAnalyzer'
...
export function useDriftMonitor() {
  ...
  const hasHighSeverity = computed(() => {
    return flaggedRegressions.value.some((r: any) => r.severity === 'high')
  })

  async function analyze(projectId: any) {
    ...
    try {
      ...
      const workspaceTypes = [
        ...new Set(evals.map((e: any) => e.workspaceType || 'unknown').filter(Boolean))
      ]

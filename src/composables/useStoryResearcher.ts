...
function filterRelevant(entities: any, premise: any, isShortTerm: any) {
  ...
  if (isShortTerm && premise) {
    ...
    const scored = entities
      .map((e: any) => {
        ...
        const score = keywords.reduce((acc: any, kw: any) => {
    ...
    return scored.map((s: any) => s.entity).slice(0, MAX_EVIDENCE_ITEMS)
  }
...
export function useStoryResearcher() {
  ...
  async function gatherEvidence(projectId: any, goal: any) {
    ...
    try {
      ...
      if (finalPlotThreads.length > 0) {
        finalPlotThreads.forEach((pt: any) => {

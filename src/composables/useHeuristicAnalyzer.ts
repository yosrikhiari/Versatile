...
const BEAT_PATTERNS = [
  ...
  {
    label: 'Rising Action',
    markers: ['meanwhile', 'but then', 'however', 'as the days', 'soon', 'gradually']
  },
  {
    label: 'Midpoint',
    markers: [
...
function computeSentenceScores(text: string): number[] {
...
function computeEmotionScores(text: string) {
  const lower = text.toLowerCase()
...
function computeStructuralBeats(text: string) {
  const lower = text.toLowerCase()
  ...
  return BEAT_PATTERNS.map((beat) => {
    ...
    for (const marker of beat.markers) {
      const count = (
        lower.match(new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')) || []
      ).length
...
export function useHeuristicAnalyzer() {
  // Shape is whatever analyzeScene produces — kept in sync automatically.
  ...
  function analyzeScene(text: string) {
    ...
    const result = {
      ...
      metrics: {
        ...
        dominantEmotion:
          emotionScores.length > 0 ? emotionScores.sort((a, b) => b.score - a.score)[0] : null,

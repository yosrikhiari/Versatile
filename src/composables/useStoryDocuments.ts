...
function getRelationshipLabel(type: any) {
  return relationshipLabels[type as keyof typeof relationshipLabels] || type
}
...
function lookupById(map: any, id: any) {
...
function resolveEntityName(type: any, id: any, maps: any) {
  switch (type) {
    case 'character':
...
async function generateCharactersDoc(projectId: any) {
...
function generateStyleGuideDoc() {
  ...
  if (domainFound) {
    lines.push(`- Voice note: ${DOMAIN_LABELS[domainFound as keyof typeof DOMAIN_LABELS]}`)
  }

  return lines.join('\n')
}

async function generateRejectedPatternsDoc(projectId: any) {
  const doc = await getAllStoryDocuments(projectId)
  const rejectedDoc = doc.find((d: any) => d.docType === DOC_TYPES.REJECTED_PATTERNS)
...
function summarizeExcerpt(content: any, wordLimit = 45) {
...
function generateStorySoFarDoc() {
  const manuscriptStore = useManuscriptStore()
  const sections = (manuscriptStore.sortedSections || []) as any[]
  const subsections = (manuscriptStore.subsections || []) as any[]
...
function splitAuthorZone(content: any) {
...
async function buildStoryContextDoc(projectId: any, existingAuthorZone: any) {
  const authorZone =
...
async function getStoryContextDoc(projectId: any) {
  if (!projectId) return null
  return getDocument(projectId, DOC_TYPES.STORY_CONTEXT)
}

async function saveStoryContextDoc(projectId: any, content: any) {
  if (!projectId) return
...
async function rebuildStoryContextDoc(projectId: any) {
  if (!projectId) return ''
...
async function loadDocuments(projectId: any) {
  return getAllStoryDocuments(projectId)
}

async function getDocument(projectId: any, docType: any) {
  const docs = await getAllStoryDocuments(projectId)
  return docs.find((d: any) => d.docType === docType) || null
}

async function regenerateDocument(projectId: any, docType: any) {
  if (!projectId) return
  let content = ''
  switch (docType) {
...
async function getDocumentContext(docType: any) {
  const projectStore = useProjectStore()
  const doc = await getDocument(projectStore.currentProjectId, docType)
  return doc?.content || ''
}

async function logRejectedPattern(projectId: any, pattern: any) {
  if (!projectId) return

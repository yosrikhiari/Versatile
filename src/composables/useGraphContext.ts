...
export function useGraphContext() {
  ...
  function parseNodeId(nodeId: any) {
  ...
  function buildNodeId(type: any, id: any) {
    return `${getEntityTypePrefix(type)}-${id}`
  }

  function getEntityName(type: any, id: any) {
    switch (type) {
      case 'character': {
  ...
  function serializePath(pathNodes: any) {
  ...
  async function getRelationshipContext(entityIds: any, depth: any = 2) {
    ...
    const queue = startNodeIds.map((startId: any) => ({
      nodeId: startId,

...
function isValidEntityName(name: string) {
...
function extractNameFromPattern(text: string, pattern: string) {
...
function extractPotentialEntities(text: string) {
  ...
  while ((match = capitalizedPattern.exec(text)) !== null) {
    ...
    const after = text
      .substring(match.index + name.length, match.index + name.length + 50)
      .toLowerCase()
    ...
    if (
      after.startsWith(' is ') ||
      after.startsWith(' was ') ||
      after.startsWith(' are ') ||
      after.startsWith(' were ')
    ) {
      characterScore += 1
    }
  ...
  const filteredCharacters = [...characters].filter((name: string) => {
  ...
  const filteredLocations = [...locations].filter((name: string) => {
    if (!isValidEntityName(name)) return false
    const lower = name.toLowerCase()
    if (COMMON_WORDS.has(lower)) return false
    for (const verb of VERB_INDICATORS) {

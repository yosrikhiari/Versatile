...
export function useEvalExport() {
  ...
  async function fetchFullEvalHistory(projectId: any) {
    ...
    const workspaceTypes = Object.values(WTypes)
    const workspaceType = workspaceTypes.includes(category) ? category : WTypes.CREATIVE
    ...
    const content: any = {}
    subsections.forEach((sub: any, i: any) => {
  ...
  async function downloadEvalHistory(projectId: any) {

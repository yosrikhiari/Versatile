...
export function useGraphContext() {
  ...
  function parseNodeId(nodeId) {
    ...
    let type
    switch (prefix) {
      case 'char': type = 'character'; break
      case 'loc': type = 'location'; break
      case 'thread': type = 'plotThread'; break
      default: return null
    }
  ...
  async function getRelationshipContext(entityIds, depth = 2) {
    ...
    while (queue.length > 0) {
      ...
      for (const neighbor of neighbors) {
        ...
        const newPath = [...current.path, { nodeId: neighbor.nodeId, relationship: neighbor.relationship }]
    ...
    const serializedPaths = allPaths

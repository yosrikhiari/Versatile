...
export function useStateSummarizer() {
  ...
  function snapshotToContextString(snapshot: any) {
    if (!snapshot) return ''
    const parts = []
  ...
  function snapshotToRecap(snapshot: any) {
    if (!snapshot) return ''
    const parts = []
    ...
    const resolved = snapshot.updatedThreads?.filter((t: any) => t.includes('resolved')).length || 0

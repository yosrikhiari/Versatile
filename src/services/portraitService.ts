...
export async function generateSDPrompt(character: CharacterInfo): Promise<string> {
...
Generate ONLY the SD prompt for this character portrait.`
  ...
  try {
    const prompt: string = await ollamaGenerate(userPrompt, systemPrompt)
    return prompt.trim()
  } catch (error) {
...
function buildFallbackPrompt(character: CharacterInfo): string {
...
export async function generatePortrait(character: CharacterInfo, projectId: string): Promise<PortraitResult> {
  try {
    ...
    const base64Image = data.images[0] as string

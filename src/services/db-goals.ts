...
export async function getDailyGoal(projectId: string) {
  const today = getTodayDateString()
  return db.dailyGoals.where({ projectId, date: today }).first()
}

export async function setDailyGoal(projectId: string, goalWords: number) {
  const today = getTodayDateString()
...
export async function updateDailyWordCount(projectId: string, wordCount: number) {
  const today = getTodayDateString()
  const existing = await db.dailyGoals.where({ projectId, date: today }).first()
  if (existing) {
...
function dateDiff(dateStr1: string, dateStr2: string): number {
...
export async function getLastSessionData(projectId: string) {
  const entries: Array<{ date: string; wordCount: number }> = await db.dailyGoals
    .where('projectId')
    .equals(projectId)
    .filter((e: any) => e.wordCount > 0)

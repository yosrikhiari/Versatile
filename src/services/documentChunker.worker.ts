...
function preSplitText(text: string, maxSize: number): string[] {
...
function extractTags(text: string, maxTags = 10): string[] {
  ...
  const unigramScore = (word: string, freq: number): number => {
  ...
  const bigramScore = (phrase: string, freq: number): number => {
    if (freq < 2) return 0
...
function mergeSmallChunks(chunks: Chunk[], minSentences: number): Chunk[] {
...
function groupSentencesByParagraph(sentences: string[], text: string): SentenceGroup[] {
...
function findHeadingBreaks(sentences: string[], text: string): number[] {
  const breaks = [0]
...
function applyBreaks(sentences: string[], breaks: number[]): SentenceGroup[] {
  const groups: SentenceGroup[] = []
...
function sizeBasedChunk(text: string, maxChunkSize: number): TextChunk[] {
...
self.onmessage = function (e: MessageEvent<WorkerRequest>) {
  ...
  try {
    const fn = methodMap[method] as ((...args: unknown[]) => unknown) | undefined

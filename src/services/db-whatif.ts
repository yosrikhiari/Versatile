...
export async function directDivergentSections(projectId: string, sourceBranchId: string, sectionIds: string[]) {
  ...
  const sourceSections = await db.sections
    ...
    .filter((s: any) => sectionIds.includes(s.id))
    .sortBy('order')

  const allSourceSubsections = await db.subsections
    .where({ projectId, branchId: sourceBranchId })
    .filter((s: any) => sectionIds.includes(s.sectionId))
    .toArray()
...
export async function deleteWhatIfBranch(branchId: string) {
...
export async function acceptDivergence(branchId: string) {
  const branch = await db.branches.get(branchId)

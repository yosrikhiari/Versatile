  ...
  const projectId = await db.transaction(
    ...
    async () => {
      ...
      if (data.characters?.length > 0) {
        const chars = data.characters.map((c: any) => ({ ...c, id: undefined, projectId: id }))
        await db.characters.bulkAdd(chars)
      }

      if (data.locations?.length > 0) {
        const locs = data.locations.map((l: any) => ({ ...l, id: undefined, projectId: id }))
        await db.locations.bulkAdd(locs)
      }

      if (data.plotThreads?.length > 0) {
        const threads = data.plotThreads.map((t: any) => ({ ...t, id: undefined, projectId: id }))
        await db.plotThreads.bulkAdd(threads)
      }

      if (data.relationships?.length > 0) {
        const rels = data.relationships.map((r: any) => ({ ...r, id: undefined, projectId: id }))
        await db.characterRelationships.bulkAdd(rels)
      }

      if (data.storyElements?.length > 0) {
        const elems = data.storyElements.map((e: any) => ({ ...e, id: undefined, projectId: id }))
        await db.storyElements.bulkAdd(elems)
      }

      if (data.sparkHistory?.length > 0) {
        const history = data.sparkHistory.map((h: any) => ({ ...h, id: undefined, projectId: id }))
      ...
      if (data.snippets?.length > 0) {
        const snippets = data.snippets.map((s: any) => ({ ...s, id: undefined, projectId: id }))
        await db.snippets.bulkAdd(snippets)
      }

      if (data.volumes?.length > 0) {
        const vols = data.volumes.map((v: any) => ({ ...v, id: undefined, projectId: id }))
        await db.volumes.bulkAdd(vols)
      }

      if (data.version >= 3 && data.volumeEntities?.length > 0) {
        const entities = data.volumeEntities.map((ve: any) => ({
          ...ve,
          id: undefined,
          projectId: id,
      ...
      if (data.version >= 3 && data.graphEdges?.length > 0) {
        const edges = data.graphEdges.map((edge: any) => ({
          ...edge,
          id: undefined,
          projectId: id,
      ...
      if (data.version >= 4) {
        if (data.sections?.length > 0) {
          const secs = data.sections.map((s: any) => ({ ...s, id: undefined, projectId: id }))
          await db.sections.bulkAdd(secs)
        }
        if (data.subsections?.length > 0) {
          const subs = data.subsections.map((s: any) => ({ ...s, id: undefined, projectId: id }))
          await db.subsections.bulkAdd(subs)
        }
      } else {
        if (data.chapters?.length > 0) {
          const secs = data.chapters.map((c: any) => ({
            projectId: id,
            title: c.title,
            summary: c.summary,
            order: c.order,
        ...
        if (data.scenes?.length > 0) {
          const subs = data.scenes.map((s: any) => ({
            projectId: id,

...
| `BaseSelect` | `modelValue` · `options[{value,label,disabled?}]`* · `label` · `placeholder` · `hint` · `error` · `size` sm·md · `disabled` · `ariaLabel` | `update:modelValue` | one-of-N choice from a list too long for `BaseSegmented`; native `<select>` with the design-system chevron and `BaseField`'s message row |

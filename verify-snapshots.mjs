...
async function runTests() {
  ...
  try {
    ...
    await db.open()
    console.log('PASS: IndexedDB opened (version 8 with snapshots table)\n')
  } catch (err) {
  ...
  console.log(`  For chapterId ${chapterId}: ${filteredSnaps.length}`)
  ...
  console.log(`  Scene content before revert: "${beforeRevert.content}"`)
  ...
  if (sceneSnaps.length >= 1 && sceneSnaps.some(s => s.id === sceneSnapId)) {
    console.log(`PASS: Scene-level snapshot saved (${sceneSnaps.length} for sceneId ${sceneId})`)
  } else {
  ...
  if (remaining.length === 0) {
    console.log('PASS: All snapshots cleaned up')
  } else {
    console.log(`FAIL: ${remaining.length} snapshots remain`)
  }

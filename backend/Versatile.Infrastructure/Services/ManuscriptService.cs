...
public class ManuscriptService : IManuscriptService
{
    ...
    public async Task<List<ManuscriptDto>> GetAllAsync(Guid storyId, Guid userId)
    {
    ...
    public async Task<ManuscriptDto> GetByIdAsync(Guid id, Guid userId)
    {
        var manuscript = await _db.Manuscripts.Include(m => m.Story).FirstOrDefaultAsync(m => m.Id == id && m.Story!.UserId == userId);
        return manuscript is null ? throw new KeyNotFoundException("Manuscript not found") : ToDto(manuscript);
    }

    public async Task<ManuscriptDto> CreateAsync(Guid storyId, CreateManuscriptRequest request, Guid userId)
    {
        await EnsureStoryAccess(storyId, userId);
    ...
    public async Task<ManuscriptDto> UpdateAsync(Guid id, UpdateManuscriptRequest request, Guid userId)
    {
        var manuscript = await _db.Manuscripts.Include(m => m.Story).FirstOrDefaultAsync(m => m.Id == id && m.Story!.UserId == userId);

...
public class DailyGoalService : IDailyGoalService
{
    ...
    public async Task<List<DailyGoalDto>> GetAllAsync(Guid storyId, Guid userId)
    {
    ...
    public async Task<DailyGoalDto> GetByIdAsync(Guid id, Guid userId)
    {
        var goal = await _db.DailyGoals.Include(d => d.Story).FirstOrDefaultAsync(d => d.Id == id && d.Story!.UserId == userId);
        return goal is null ? throw new KeyNotFoundException("DailyGoal not found") : ToDto(goal);
    }

    public async Task<DailyGoalDto> CreateAsync(Guid storyId, CreateDailyGoalRequest request, Guid userId)
    {
        await EnsureStoryAccess(storyId, userId);
    ...
    public async Task<DailyGoalDto> UpdateAsync(Guid id, UpdateDailyGoalRequest request, Guid userId)
    {
        var goal = await _db.DailyGoals.Include(d => d.Story).FirstOrDefaultAsync(d => d.Id == id && d.Story!.UserId == userId);

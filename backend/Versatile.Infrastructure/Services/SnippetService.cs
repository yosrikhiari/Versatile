...
public class SnippetService : ISnippetService
{
    ...
    public async Task<List<SnippetDto>> GetAllAsync(Guid storyId, Guid userId)
    {
    ...
    public async Task<SnippetDto> GetByIdAsync(Guid id, Guid userId)
    {
        var snippet = await _db.Snippets.Include(s => s.Story).FirstOrDefaultAsync(s => s.Id == id && s.Story!.UserId == userId);
        return snippet is null ? throw new KeyNotFoundException("Snippet not found") : ToDto(snippet);
    }

    public async Task<SnippetDto> CreateAsync(Guid storyId, CreateSnippetRequest request, Guid userId)
    {
        await EnsureStoryAccess(storyId, userId);
    ...
    public async Task<SnippetDto> UpdateAsync(Guid id, UpdateSnippetRequest request, Guid userId)
    {
        var snippet = await _db.Snippets.Include(s => s.Story).FirstOrDefaultAsync(s => s.Id == id && s.Story!.UserId == userId);

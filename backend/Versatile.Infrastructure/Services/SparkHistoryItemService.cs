...
public class SparkHistoryItemService : ISparkHistoryItemService
{
    ...
    public async Task<List<SparkHistoryItemDto>> GetAllAsync(Guid storyId, Guid userId)
    {
    ...
    public async Task<SparkHistoryItemDto> GetByIdAsync(Guid id, Guid userId)
    {
        var item = await _db.SparkHistoryItems.Include(s => s.Story).FirstOrDefaultAsync(s => s.Id == id && s.Story!.UserId == userId);
        return item is null ? throw new KeyNotFoundException("SparkHistoryItem not found") : ToDto(item);
    }

    public async Task<SparkHistoryItemDto> CreateAsync(Guid storyId, CreateSparkHistoryItemRequest request, Guid userId)
    {
        await EnsureStoryAccess(storyId, userId);
    ...
    public async Task<SparkHistoryItemDto> UpdateAsync(Guid id, UpdateSparkHistoryItemRequest request, Guid userId)
    {
        var item = await _db.SparkHistoryItems.Include(s => s.Story).FirstOrDefaultAsync(s => s.Id == id && s.Story!.UserId == userId);

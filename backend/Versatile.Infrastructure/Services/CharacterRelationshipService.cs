...
public class CharacterRelationshipService : ICharacterRelationshipService
{
    ...
    public async Task<List<CharacterRelationshipDto>> GetAllAsync(Guid storyId, Guid userId)
    {
    ...
    public async Task<CharacterRelationshipDto> GetByIdAsync(Guid id, Guid userId)
    {
        var rel = await _db.CharacterRelationships.Include(r => r.Story).FirstOrDefaultAsync(r => r.Id == id && r.Story!.UserId == userId);
        return rel is null ? throw new KeyNotFoundException("CharacterRelationship not found") : ToDto(rel);
    }

    public async Task<CharacterRelationshipDto> CreateAsync(Guid storyId, CreateCharacterRelationshipRequest request, Guid userId)
    {
        await EnsureStoryAccess(storyId, userId);
    ...
    public async Task<CharacterRelationshipDto> UpdateAsync(Guid id, UpdateCharacterRelationshipRequest request, Guid userId)
    {
        var rel = await _db.CharacterRelationships.Include(r => r.Story).FirstOrDefaultAsync(r => r.Id == id && r.Story!.UserId == userId);

...
public sealed class OpenAiChatProvider : IChatProvider
{
    ...
    public OpenAiChatProvider(HttpClient http, string apiKey)
    {
        _http = http;
        _apiKey = apiKey;
    }
    ...
    public async Task<TestConnectionResult> TestConnectionAsync(string model, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var json = await response.Content.ReadFromJsonAsync<OpenAiModelsResponse>(JsonOptions, ct);
            return new TestConnectionResult(true, json?.Data?.FirstOrDefault()?.Id, null);
        }
        catch (Exception ex)
        {
    ...
    private sealed record OpenAiStreamChunk(IReadOnlyList<OpenAiChoice>? Choices);
    private sealed record OpenAiChoice(OpenAiDelta? Delta, string? FinishReason);
    private sealed record OpenAiDelta(string? Content);
    private sealed record OpenAiModelInfo(string Id);
    private sealed record OpenAiModelsResponse(IReadOnlyList<OpenAiModelInfo>? Data);
}

...
public class PlotThreadService : IPlotThreadService
{
    ...
    public async Task<List<PlotThreadDto>> GetAllAsync(Guid storyId, Guid userId)
    {
    ...
    public async Task<PlotThreadDto> GetByIdAsync(Guid id, Guid userId)
    {
        var plotThread = await _db.PlotThreads.Include(p => p.Story).FirstOrDefaultAsync(p => p.Id == id && p.Story!.UserId == userId);
        return plotThread is null ? throw new KeyNotFoundException("PlotThread not found") : ToDto(plotThread);
    }

    public async Task<PlotThreadDto> CreateAsync(Guid storyId, CreatePlotThreadRequest request, Guid userId)
    {
        await EnsureStoryAccess(storyId, userId);
    ...
    public async Task<PlotThreadDto> UpdateAsync(Guid id, UpdatePlotThreadRequest request, Guid userId)
    {
        var plotThread = await _db.PlotThreads.Include(p => p.Story).FirstOrDefaultAsync(p => p.Id == id && p.Story!.UserId == userId);

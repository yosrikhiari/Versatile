...
// ---- 1. tokens documented -------------------------------------------------------------
{
  ...
  const defined = [...new Set([...css.matchAll(/(--vers-[a-z0-9_-]+)\s*:/g)].map((m) => m[1]))].sort()
  ...
  if (missing.length) fail('tokens-documented', `defined in src/style.css but absent from docs/DESIGN-TOKENS.md: ${missing.join(', ')}`)
  if (defined.length < 40) fail('tokens-documented', `only ${defined.length} tokens found; the scan is broken, not the stylesheet`)
}
...
// ---- 2. primitives have stories --------------------------------------------------------
{
  ...
  const missing = primitives.filter((f) => !existsSync(join(dir, f.replace(/\.vue$/, '.stories.js'))))
  if (missing.length) fail('primitives-have-stories', `no story for: ${missing.join(', ')} (add src/components/ui/<Name>.stories.js)`)
  if (primitives.length < 10) fail('primitives-have-stories', `only ${primitives.length} primitives found; the scan is broken`)
}
...
// ---- 3. hex ratchet ------------------------------------------------------------------
{
  ...
  for (const [file, n] of Object.entries(counts)) {
    ...
    if (n > allowed) fail('hex-ratchet', `${file}: ${n} hex colour literal(s), baseline allows ${allowed}. Use a --vers-* token (docs/DESIGN-TOKENS.md).`)
  }
...
// ---- 4. agent files point at AGENTS.md -------------------------------------------------
{
  ...
  for (const rel of ['CLAUDE.md', 'GEMINI.md', 'opencode.json', '.cursor/rules/agents.mdc', '.github/copilot-instructions.md']) {
    if (!existsSync(join(root, rel))) fail('agent-files', `${rel} is missing`)
    else if (!read(rel).includes('AGENTS.md')) fail('agent-files', `${rel} does not point at AGENTS.md`)
  }
  if (existsSync(join(root, 'CLAUDE.md')) && /^## /m.test(read('CLAUDE.md')))
    fail('agent-files', 'CLAUDE.md has its own sections; it must stay a pointer (@AGENTS.md), not a second rule file')
}

...
function splitByPeriod(evals, recentFraction) {
...
function computeOverallTrend(evals) {
  const sorted = [...evals].sort((a, b) => {
    const ta = a.timestamp || ''
    const tb = b.timestamp || ''
    return ta < tb ? -1 : ta > tb ? 1 : 0
...
function analyzeWorkspace(evals, workspaceType, options) {
...
`)
  ...
  if (
    report.summary.dimensionsWithRegression === 0 &&
    report.summary.dimensionsWithImprovement === 0
  ) {
  ...
  for (const r of report.flaggedItems.improvements) {
    ...
    log(
      `   Delta: +${Math.abs(r.delta)}  |  Baseline: ${r.baseline.mean} → Recent: ${r.recent.mean}`
    )
    log(`   ${r.recommendation}`)
    log('')
  }

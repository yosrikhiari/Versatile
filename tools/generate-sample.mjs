...
console.log(`Model: ${MODEL} | target: ~${TARGET_WORDS} words/scene | 2 scenes`)
const scenes = []
...
for (let i = 0; i < scenes.length; i++) {
  const fixture = {
    ...
    chapterLog: i === 0 ? '' : `Scene 1 "${scenes[0].title}": ${scenes[0].prose.slice(0, 600)}…`,
...
writeFileSync(`reports/sample-${stamp}.json`, JSON.stringify(report, null, 2))
...
`
writeFileSync(`reports/sample-${stamp}.md`, md)
console.log(`Saved reports/sample-${stamp}.json + .md`)

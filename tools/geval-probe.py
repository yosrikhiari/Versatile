...
for idx in SCENES:
    s = corpus[idx]
    for dname, (target, fn) in DEFECTS.items():
        draft = fn(s["draft"], s["storyBible"])
        for dim in DIMS:
            score, ev, dist, n = call(dim, s, draft)
            rows.append({"scene": idx, "defect": dname, "target": target, "dim": dim, "score": score, "ev": ev, "dist": dist})
            print(idx, dname, dim, score, None if ev is None else round(ev, 2), dist, flush=True)
